...
class SolarEnergyFlowCoordinator(DataUpdateCoordinator[FlowState]):
    ...
    def _apply_grid_limiter(
        ...
        if limiter_active:
            if options.limiter_type == GRID_LIMITER_TYPE_IMPORT:
                if prev_limiter_state == GRID_LIMITER_STATE_LIMITING_IMPORT:
                    if inputs.grid_power < options.limiter_limit_w - options.limiter_deadband_w:
                        new_limiter_state = GRID_LIMITER_STATE_NORMAL
                    else:
                        new_limiter_state = GRID_LIMITER_STATE_LIMITING_IMPORT
                elif inputs.grid_power > options.limiter_limit_w + options.limiter_deadband_w:
                    new_limiter_state = GRID_LIMITER_STATE_LIMITING_IMPORT
        ...
        elif options.limiter_enabled and inputs.grid_power is None:
    ...
    async def _maybe_write_output(
        ...
        if desired_output is None:
            return OutputWriteResult(output=self._last_output, status="", write_failed=False)
        ...
        if should_write or self._last_output is None:
            self._last_output = final_output

        return OutputWriteResult(output=self._last_output, status="", write_failed=write_failed)
    ...
    def _calculate_output_plan(
        ...
        pv_for_pid = limiter_result.pv_for_pid
        sp_for_pid = limiter_result.sp_for_pid
        ...
        if runtime_mode == RUNTIME_MODE_HOLD:
            held_output = self._last_output if self._last_output is not None else options.min_output
            self._limiter_state = GRID_LIMITER_STATE_NORMAL
        ...
        if runtime_mode == RUNTIME_MODE_MANUAL_OUT:
            ...
            self._previous_runtime_mode = runtime_mode
            self._log_runtime_mode_change(prev_runtime_mode, runtime_mode, setpoint.manual_sp_value, manual_sp_display_value)
            self._last_output = manual_out_value
            return OutputPlan(
        ...
        if limiter_result.limiter_state == GRID_LIMITER_STATE_NORMAL and options.pid_deadband > 0 and abs(error) < options.pid_deadband:
            error = 0.0
